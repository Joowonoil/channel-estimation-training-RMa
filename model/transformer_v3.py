...
from model.adapter import Adapter # Adapter Module 임포트
...
class Transformer(nn.Module):
    def __init__(self, length, channels, num_layers, d_model, n_token, n_head, dim_feedforward, dropout,
                 activation="relu", cond_net=None, adapter_bottleneck_dim=None, adapter_dropout=None): # Adapter 관련 파라미터 추가
        ...
        for _ in range(self._num_layers):
            layer = TransformerLayer(self._d_model, self._n_token, n_head, dim_feedforward, dropout,
                                     ...
                                     adapter_bottleneck_dim=adapter_bottleneck_dim, # Adapter 관련 파라미터 전달
...
class TransformerLayer(nn.Module):
    def __init__(self, d_model, n_token, n_head, dim_feedforward, dropout, activation="relu",
                 cond_d_model=None, cond_n_token=None, adapter_bottleneck_dim=None, adapter_dropout=None): # Adapter 관련 파라미터 추가
        ...
        self.adapter1 = None
        self.adapter2 = None
        if adapter_bottleneck_dim is not None and adapter_dropout is not None:
             self.adapter1 = Adapter(d_model, adapter_bottleneck_dim, adapter_dropout)
             self.adapter2 = Adapter(d_model, adapter_bottleneck_dim, adapter_dropout)
    ...
    def forward(self, input, cond=None):
        # # Multi-head attention (Post-normalization)
        # x = input
        # key = value = input
        ...
        # if self.adapter1 is not None: # Adapter 1 적용
        #     x = self.adapter1(x)
        ...
        # if self.adapter2 is not None: # Adapter 2 적용
        #     x = self.adapter2(x)
        ...
        # Pre-normalization
        # Self-attention
        norm_x = self.norm1(input)
        x2, _ = self.mha(query=norm_x, key=norm_x, value=norm_x, query_pos=self.pe, key_pos=self.pe,
                         attn_mask=None, need_weights=False, average_attn_weights=False)
        x = input + self.dropout1(x2)
        ...
        # Cross-attention
        norm_x = self.norm2(x)
        x2, _ = self.cond_mha(query=norm_x, key=cond, value=cond, query_pos=self.pe, key_pos=self.cond_pe,
                              attn_mask=None, need_weights=False, average_attn_weights=False)
        x = x + self.dropout2(x2)
        ...
        # Feedforward
        norm_x = self.norm3(x)
        x2 = self.ffnn_linear2(self.ffnn_dropout(self.activation(self.ffnn_linear1(norm_x))))
        x = x + self.dropout3(x2)
        return x
