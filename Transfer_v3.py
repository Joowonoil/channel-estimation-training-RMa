...
from model.estimator_v3 import Estimator_v3 # 모델 Estimator_v3 클래스 임포트
...
# from model.transformer_v2 import Transformer # v2 Transformer 모델 임포트 (v3에서는 사용 안 함)
...
class TransferLearningEngine:
    ...
    def load_model(self):
        # Estimator_v3 클래스는 내부에서 모델 로딩 및 프리징을 처리합니다.
        self._estimator = Estimator_v3(self._conf_file).to(self._device)

        model_load_mode = self._conf['training'].get('model_load_mode', 'pretrained')

        if model_load_mode == 'finetune':
            load_path = self._conf['training'].get('load_model_path')
            if load_path:
                try:
                    # load_model_path가 설정되어 있으면 해당 경로에서 모델 상태 로드
                    model_path = Path(load_path)
                    if model_path.exists():
                        self._estimator.load_state_dict(torch.load(model_path))
                        print(f"Model loaded successfully from {model_path} for finetuning.")
                    else:
                        print(f"Error: Model file not found at {model_path} for finetuning.")
                        print("Please check 'load_model_path' in the config file.")
                        # 파일이 없으면 프로그램 종료 또는 다른 처리 필요 시 여기에 추가
                        raise FileNotFoundError(f"Model file not found at {model_path}") # 파일이 없으면 오류 발생
                except Exception as e:
                    print(f"Error loading model from {load_path}: {e}")
                    print("Please check 'load_model_path' and the model file.")
                    # 로드 실패 시 프로그램 종료 또는 다른 처리 필요 시 여기에 추가
                    raise RuntimeError(f"Failed to load model from {load_path}") # 로드 실패 시 오류 발생
            else:
                print("Error: 'load_model_path' is not specified in the config file for 'finetune' mode.")
                print("Please specify the path to the saved model.")
                # load_model_path가 설정되지 않았으면 프로그램 종료 또는 다른 처리 필요 시 여기에 추가
                raise ValueError("'load_model_path' must be specified for 'finetune' mode.") # 경로 미설정 시 오류 발생
        elif model_load_mode == 'pretrained':
            # 'pretrained' 모드이면 Estimator_v3 내부 로직에 따라 사전 학습된 가중치 로드
            print(f"Estimator_v3 model initialized and pretrained weights loaded.")
        else:
            print(f"Warning: Unknown model_load_mode '{model_load_mode}'. Initializing with pretrained weights.")
            # 알 수 없는 모드이면 사전 학습된 가중치로 초기화
            print(f"Estimator_v3 model initialized and pretrained weights loaded.")
    ...
    def set_optimizer(self):
        ...
        # Estimator_v3에서는 Adapter 파라미터만 훈련 가능하도록 설정되어 있습니다.
        ch_params = [p for n, p in self._estimator.named_parameters() if p.requires_grad] # 훈련 가능한 파라미터 가져오기
    ...
    def train(self):
        ...
        for it, data in enumerate(self._dataloader):
            ...
            torch.nn.utils.clip_grad_norm_(self._estimator.parameters(), max_norm=self._conf['training']['max_norm']) # 그래디언트 클리핑 (전체 모델 파라미터에 적용)
    ...
    def save_model(self, file_name):
        ...
        torch.save(self._estimator.state_dict(), path / (file_name + '.pt')) # 모델 저장 (state_dict 저장)
