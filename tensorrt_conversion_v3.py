...
from model.estimator_v3 import Estimator_v3 # v3 모델 임포트
...
def export_onnx_model(file_name, input_shape, batch_size):
    # v3 모델 로딩 및 초기화
    conf_file = 'config_transfer_v3.yaml' # v3 설정 파일 사용
    model = Estimator_v3(conf_file).cuda().eval()
    ...
    # The freezing/unfreezing logic is for training, not strictly necessary for ONNX export,
    # but keeping it for consistency with the training setup.
    for param in model.ch_tf.parameters():
        param.requires_grad = False

    for name, param in model.ch_tf.named_parameters():
        if 'adapter' in name:
...
def torch_inference(file_name, input_tensor):
    ...
    # v3 모델 로딩 및 초기화
    conf_file = 'config_transfer_v3.yaml' # v3 설정 파일 사용
    model = Estimator_v3(conf_file=conf_file).cuda().eval()
    ...
    # The freezing/unfreezing logic is for training, not strictly necessary for inference,
    # but keeping it for consistency with the training setup.
    for param in model.ch_tf.parameters():
        param.requires_grad = False
...
if __name__ == "__main__":
    ...
    # Compile estimator
    file_name = "InF_Nlos_RMa_Large_estimator_PreLN_LoRA_InF"
